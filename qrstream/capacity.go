...
const (
	Q Level = iota // ~25% recovery; zero value and default
	L              // ~7%
	M              // ~15%
	H              // ~30%
)
...
func (l Level) recovery() qrcode.RecoveryLevel {
	// skip2 names Q-level "High" and H-level "Highest".
	switch l {
	case L:
		return qrcode.Low
	case M:
...
// byteCapacity is the maximum number of data bytes in 8-bit byte mode
// per symbol version and EC level (ISO/IEC 18004). Columns are L, M, Q, H.
...
// Capacity returns the data bytes a symbol of the given version and EC
// level can carry in byte mode.
func Capacity(version int, level Level) int {
	...
	return byteCapacity[version][column]
}
